...
const main = async (download = false) => {
  ...
  $('.guide.subSections .subSection.detailBox').each(function subSections() {
    const title = $(this).find('.subSectionTitle').text().trim()
      .split(' ');
    const type = title[1];
    const categories = [];

    $(this).find('.subSectionDesc .bb_blockquote').each(function quotes() {
      const items = [];

      $(this).find('.bb_table_tr').each(function tables() {
    ...
    if (categories.length > 0) {
      if (!runeData[title[0]] && typeof runeData[title[0]] !== 'object') {
        runeData[title[0]] = [];
      }
...
main().then((images) => fs.writeFile(path.normalize('./src/assets/runes_tree.json'), JSON.stringify(sortIntoTree(images), null, 2))).catch(console.log);
